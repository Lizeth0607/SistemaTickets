...
const Equipos = () => {
   ...
   */
   ...
   const onSubmit = (data) => {
      ...
      if (captura) {
         agregaEquipo(discapacidadCopy);
      } else {
         actualizaEquipo(discapacidadCopy);
      }
      reset();
   };
   ...
   const formik = useFormik({
      ...
      validate,
      onSubmit: (Empleados) => {
         let discapacidadCopy = Object.assign({}, Empleados);
         if (captura) {
            agregaEquipo();
         } else {
            actualizaEquipo(discapacidadCopy);
   ...
   return (
      <div>
         ...
         <DataTable value={lstEquipos} paginator={true} rows={10} responsive={true}>
            ...
            <Column field="detalles" header={t('Equipos:label.detalles')} sortable={true}></Column>
            <Column field="compra" header={t('Equipos:label.compra')} sortable={true}></Column>
            <Column field="can_install" header={t('Equipos:label.instalacion')} sortable={true}></Column>
         ...
         <Dialog header={t('Equipos:rotulo.agregar')}  visible={dlgEquipos} modal={true} style={{ width: '50vw' }} onHide={(e) => setDlgEquipos(false)} blockScroll={false}>
            {Equipos &&
               <div>
                  <form onSubmit={handleSubmit(onSubmit)}>
                     <div className="p-fluid p-formgrid p-grid">
                        ...
                        <div className="p-field p-col-12 p-md-12"><label htmlFor="txtEsp">
                           ...
                           {{ captura } ? (
                              <InputTextarea placeholder={t('Equipos:placeholder.especificaciones')} className={formik.errors.txtEsp ? 'p-invalid' : 'p-inputtext'} value={Equipos.detalles} onChange={(e) => updateProperty('detalles', e.target.value)} rows={3} cols={20} autoResize />
                        ...
                        <div className="p-field p-col-12 p-md-12"><label htmlFor="txtInstall">
                           ...
                           {{ captura } ? (
                              <InputText id="txtInstall" placeholder={t('Equipos:placeholder.can_install')} value={Equipos.can_install} className={formik.errors.txtInstall ? 'p-invalid' : 'p-inputtext'} maxLength={45} onChange={(e) => updateProperty('can_install', e.target.value)}></InputText>
